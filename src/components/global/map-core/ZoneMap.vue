    ...
    drawBoundary(BMap, map) {
      ...
      *      这样就做出了一个经过多次西北角的闭合多边形*/
      ...
      for (let i = 0; i < this.blist.length; i++) {
          ...
          //添加多边形层并显示
          let ply = new BMap.Polygon(this.blist[i].points, { strokeWeight: 5, strokeColor: "#409EFF", fillOpacity: 0.01, fillColor: " #FFFFFF" }); //建立多边形覆盖物
      ...
      //添加遮蔽层
      let plyall = new BMap.Polygon(pArray, { strokeOpacity: 0.0000001,  strokeColor: "#f5f5f5", strokeWeight: 0.00000000001, fillColor: "#000000", fillOpacity: 0.4 }); //建立多边形覆盖物
